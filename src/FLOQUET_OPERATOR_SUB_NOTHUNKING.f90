...
SUBROUTINE FLOQUET_BOSE_HUBBARD_SUB(STATES,Q_T,U_FTY,U_MB,EIGENVALUES,INFO)
  ...
  !COMPLEX*16,       DIMENSION(:,:), ALLOCATABLE   ::U_MB_q,U_MB_qq_
...
!    write(*,*) "CUBLAS, USING THUNKING"
  ...
  OPEN(UNIT=1,FILE='/home/g/gs/gs249/Programs/ED-EntanglementSpectrum-FloquetV2/data/SP_gpu.dat',ACTION='WRITE')
#endif

  call cublas_init()

#else
  OPEN(UNIT=1,FILE='/home/g/gs/gs249/Programs/ED-EntanglementSpectrum-FloquetV2/data/SP_cpu.dat',ACTION='WRITE')
#endif
...
!
!
!     DO q = 1,N_SITES_Y!(N_SITES_Y*N_SITES_Y + N_SITES_Y)/2 !loop over blocks of total momentum_Y
!     DO q_= q+1,N_SITES_Y
!        !q  = Q_T
!        !q_ = Q_T
!
...
!        ALPHA_ = DCMPLX(1.0,0.0)
!        BETA_  = DCMPLX(0.0,0.0)
!        call cublas_zGEMM('n','n',D,D,D,alpha_,U_Y, D,conjg(U_MB),D,beta_,U_FLOQUET,D) ! U_FLOQUET IN THIS CASE IS AN AUXILIARY MATRIX
!        call cublas_zGEMM('t','n',D,D,D,alpha_,U_MB,D,U_FLOQUET,  D,beta_,U_Y,      D)
!        !U_Y = MATMUL(TRANSPOSE(U_MB),MATMUL(U_Y,CONJG(U_MB))) ! transformation to the eigen-basis of U_X, |kq,k'q'>
!#else
!        U_Y = MATMUL(TRANSPOSE(U_MB),MATMUL(U_Y,CONJG(U_MB))) ! transformation to the eigen-basis of U_X, |kq,k'q'>
!#endif
!
...
!           END DO
!        END DO
!        CALL LAPACK_ZGEEV_GPU(U_F,D,EIGENVALUES,INFO)!Eigenvectors of the full Floquet operator, stored as U_MB(:,i) in the {|kq,k'q'>}
!
!        DO i_=1,D
!           DO k_=1,D
!              U_FLOQUET(k_,i_) = U_F(i_ + (k_-1)*D)
