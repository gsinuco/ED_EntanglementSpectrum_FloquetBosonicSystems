...
SUBROUTINE QUICK_SORT_I_T(v,index_t,N)
  ...
  INTEGER, PARAMETER :: NN=500000000, NSTACK=500000
  INTEGER :: a!, cpu
  !integer :: trial
...
SUBROUTINE QUICK_SORT_I_T_DOUBLE(v,index_t,N)
  ...
  INTEGER, PARAMETER :: NN=10000000, NSTACK=5000000
  DOUBLE PRECISION :: a!, cpu
  !integer :: trial
  INTEGER :: k,i,j,jstack,l,r,istack(NSTACK),indice
  ...
  do
     if (r-l < NN) then
        ...
        k=(l+r)/2
        call swap_DOUBLE(index_t(k),index_t(l+1))
        if (v(index_t(l)) > v(index_t(r))) call swap_DOUBLE(index_t(l),index_t(r))
        if (v(index_t(l+1))>v(index_t(r))) call swap_DOUBLE(index_t(l+1),index_t(r))
        if (v(index_t(l))>v(index_t(l+1))) call swap_DOUBLE(index_t(l),index_t(l+1))
        ...
        do
           ...
           if (j < i) exit
           call swap_DOUBLE(index_t(i),index_t(j))
        end do
        index_t(l+1)=index_t(j)
        index_t(j)=indice
        jstack=jstack+2
...
END SUBROUTINE QUICK_SORT_I_T_DOUBLE
